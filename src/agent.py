...
class StockSenseAgent:
    ...
    def _validate_path(self, filepath, allowed_dir):
        """Validates that a given filepath stays within the allowed directory."""
        base_dir = os.path.realpath(allowed_dir)
        target_path = os.path.realpath(filepath)
        if os.path.commonpath([base_dir, target_path]) != base_dir:
            raise ValueError(f"Security Error: Path traversal detected - {filepath}")
        return target_path
    ...
    def scan_inventory(self, inventory_file="data/sample_inventory.csv"):
        ...
        try:
            valid_file = self._validate_path(inventory_file, "data")
            inventory = pd.read_csv(valid_file)
    ...
    def save_recommendations(self, recommendations, output_file="output/recommendations.json"):
        ...
        try:
            valid_file = self._validate_path(output_file, "output")
        except ValueError as e:
            print(f"{self.logger_prefix} ERROR: {e}")
            return
        ...
        print(f"{self.logger_prefix} Recommendations saved to {valid_file}")
