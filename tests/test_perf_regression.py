...
class TestOptimization(unittest.TestCase):
    ...
    def test_scan_inventory_optimization(self):
        ...
        # Mock read_csv
        sys.modules['agent'].pd.read_csv = MagicMock(return_value=mock_df)
        ...
        with patch('agent.datetime') as mock_datetime:
            ...
            # Mock _validate_path to avoid filesystem checks during this perf test
            agent._validate_path = MagicMock(return_value='dummy.csv')
            ...
            print(f"✓ Calls to to_datetime: {sys.modules['agent'].pd.to_datetime.call_count}")
