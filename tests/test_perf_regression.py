...
class TestOptimization(unittest.TestCase):
    ...
    def test_scan_inventory_optimization(self):
        ...
        # Import agent module to modify its mocked pandas
        import agent
        
        # Mock read_csv on the agent's pandas reference
        agent.pd.read_csv = MagicMock(return_value=mock_df)

        # Mock to_datetime (just returns the column) on the agent's pandas reference
        ...
        with patch('agent.datetime') as mock_datetime:
            ...
            # Run scan_inventory
            stock_agent = StockSenseAgent()
            # Bypass path validation for test using a mock
            stock_agent._validate_path = MagicMock(return_value='data/dummy.csv')
            stock_agent.scan_inventory('data/dummy.csv')
