...
# Use patch dict context manager to mock pandas cleanly
...
class TestOptimization(unittest.TestCase):
    ...
    def test_scan_inventory_optimization(self):
        ...
        # Ensure columns evaluates properly in conditional checking
        type(mock_df).columns = PropertyMock(return_value=['name', 'stock', 'expiry_date', 'daily_sales'])
        ...
        with patch('agent.pd') as mock_pd:
            ...
            with patch('agent.datetime') as mock_datetime:
                ...
                agent_instance.scan_inventory('data/dummy.csv')
